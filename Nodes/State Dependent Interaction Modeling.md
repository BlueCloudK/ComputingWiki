...
Aliases: state-dependent modeling, mô hình tương tác phụ thuộc trạng thái

Type: Artifact / Diagram

## Bản chất

State Dependent Interaction Modeling là artifact dùng để nhìn thấy cấu trúc, luồng hoặc quan hệ mà đọc code/tài liệu chữ khó thấy. Giá trị của nó nằm ở việc làm rõ boundary, actor, component, data flow hoặc state chứ không phải vẽ cho đẹp. Nó nối với các phần liên quan như [[State Machine]] và nhóm quyết định quanh state dependent interaction modeling.

## Dùng trong dự án để làm gì

State Dependent Interaction Modeling giúp team thống nhất hình dung trước khi sửa architecture, API, database hoặc workflow. Nó là output để review quyết định, phát hiện dependency ẩn và onboarding người mới nhanh hơn.
