...
Aliases: stack, ngăn xếp

Type: Computer Foundation

## Context / Ngữ cảnh

Stack xuất hiện như nền tảng giúp hiểu correctness, complexity, data structure, algorithm hoặc mô hình tính toán phía dưới phần mềm.

## Boundary / Ranh giới

### Nó là gì

Stack là kiến thức nền để giải thích vì sao code chạy đúng, chậm, sai ở edge case hoặc không scale khi input tăng.
...
Stack ảnh hưởng tới lựa chọn thuật toán, cấu trúc dữ liệu, test edge case và debug lỗi performance/correctness.
...
- Chưa cần đào sâu Stack nếu input nhỏ, bounded và không ảnh hưởng trực tiếp tới correctness hoặc latency người dùng
