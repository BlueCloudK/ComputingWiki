...
Aliases: hash map, bảng băm

Type: Computer Foundation

## Context / Ngữ cảnh

Hash Table xuất hiện như nền tảng giúp hiểu correctness, complexity, data structure, algorithm hoặc mô hình tính toán phía dưới phần mềm.

## Boundary / Ranh giới

### Nó là gì

Hash Table là kiến thức nền để giải thích vì sao code chạy đúng, chậm, sai ở edge case hoặc không scale khi input tăng.
...
Hash Table ảnh hưởng tới lựa chọn thuật toán, cấu trúc dữ liệu, test edge case và debug lỗi performance/correctness.
...
- Chưa cần đào sâu Hash Table nếu input nhỏ, bounded và không ảnh hưởng trực tiếp tới correctness hoặc latency người dùng
...
- bảng băm
- hash map
- Hash Table learning keyword
- Hash Table debug keyword
- Hash Table design keyword
