...
Aliases: routing algorithm, thuật toán định tuyến

Type: Computer Foundation

## Context / Ngữ cảnh

Routing Algorithm xuất hiện như nền tảng giúp hiểu correctness, complexity, data structure, algorithm hoặc mô hình tính toán phía dưới phần mềm.

## Boundary / Ranh giới

### Nó là gì

Routing Algorithm là kiến thức nền để giải thích vì sao code chạy đúng, chậm, sai ở edge case hoặc không scale khi input tăng.
...
Routing Algorithm ảnh hưởng tới lựa chọn thuật toán, cấu trúc dữ liệu, test edge case và debug lỗi performance/correctness.
...
- Chưa cần đào sâu Routing Algorithm nếu input nhỏ, bounded và không ảnh hưởng trực tiếp tới correctness hoặc latency người dùng
...
- [[Network Layer]] vì node này thường được kiểm tra cùng khi ra quyết định
