...
Aliases: B-tree, cây B

Type: Data / Database

## Context / Ngữ cảnh

B Tree xuất hiện khi hệ thống cần lưu, truy vấn, migrate, kiểm soát consistency hoặc hiểu dữ liệu qua thời gian. Nó thường nằm trong database schema, data model, query path hoặc integration payload.

## Boundary / Ranh giới

### Nó là gì

B Tree là một phần của cách dữ liệu được biểu diễn, ràng buộc, truy cập hoặc giữ đúng trong hệ thống.
...
B Tree ảnh hưởng tới database design, query performance, migration, audit/history, backup và cách debug lỗi dữ liệu.
...
- [[Index]] vì node này thường được kiểm tra cùng khi ra quyết định
- [[Tree]] vì node này thường được kiểm tra cùng khi ra quyết định
